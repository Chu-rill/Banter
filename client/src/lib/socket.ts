...
class SocketService {
  ...
  connectToMessages() {
    ...
    this.messageSocket.on('new-message', (message: Message & { isOwn: boolean }) => {
      this.emit('message-received', message);
    });

    // Handle message sent confirmation
    this.messageSocket.on('message-sent', (message: Message & { isOwn: boolean }) => {
      this.emit('message-sent', message);
    });

    // Handle typing indicators
    this.messageSocket.on('user-typing', (data: { userId: string; username: string; roomId: string }) => {
      this.emit('user-typing', data);
    });

    this.messageSocket.on('user-stopped-typing', (data: { userId: string; roomId: string }) => {
      this.emit('user-stopped-typing', data);
    });

    // Handle read receipts
    this.messageSocket.on('messages-read', (data: { userId: string; roomId: string; lastMessageId: string }) => {
      this.emit('messages-read', data);
    });

    // Handle user presence
    this.messageSocket.on('user-joined-room', (data: { userId: string; roomId: string; timestamp: Date }) => {
      this.emit('user-joined-room', data);
    });

    this.messageSocket.on('user-left-room', (data: { userId: string; roomId: string; timestamp: Date }) => {
      this.emit('user-left-room', data);
    });

    // Handle friend status changes
    this.messageSocket.on('friend-status-change', (data: { userId: string; isOnline: boolean; timestamp: Date }) => {
  ...
  // Call methods
  joinCall(roomId: string, mediaState: { video: boolean; audio: boolean; screen: boolean }) {
    if (this.callSocket) {
  ...
  sendWebRTCOffer(roomId: string, targetUserId: string, offer: RTCSessionDescriptionInit) {
    if (this.callSocket) {
      this.callSocket.emit('webrtc-offer', { roomId, targetUserId, offer });
    }
  }

  sendWebRTCAnswer(roomId: string, callerId: string, answer: RTCSessionDescriptionInit) {
    if (this.callSocket) {
      this.callSocket.emit('webrtc-answer', { roomId, callerId, answer });
    }
  }

  sendWebRTCIceCandidate(roomId: string, targetUserId: string, candidate: RTCIceCandidateInit) {
    if (this.callSocket) {
      this.callSocket.emit('webrtc-ice-candidate', { roomId, targetUserId, candidate });
    }
  }

  updateMediaState(roomId: string, mediaState: { video: boolean; audio: boolean; screen: boolean }) {
    if (this.callSocket) {
